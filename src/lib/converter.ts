...
const parseHtmlToQuestions = (html: string): Question[] => {
    ...
    for (const el of elements) {
        ...
        if (isNewQuestion) {
            ...
            let optionsOnThisLine: { key: string, text: string, index: number, matchLength: number }[] = [];
            ...
            while ((match = optionRegex.exec(textContent)) !== null) {
                ...
                optionsOnThisLine.push({ key, text: '', index: match.index, matchLength: match[0].length });
            }
